...
이러한 페이지 테이블을 잘 설정하면 물리 메모리의 단편화되어 있는 영역을 프로세스의 가상 주소 공간에서는 하나의 큰 영역처럼 보이게 하여 단편화 문제를 해결할 수 있다.

가상 주소 공간은 프로세스별로 만들어진다.
...
<details>
<summary>디맨드 페이징</summary>

프로세스는 생성될 때 혹은 그 후에 mmap() 시스템 콜로 프로세스에 메모리를 할당한다. 이때 커널이 필요한 영역을 메모리에 확보하고 커널이 페이지 테이블을 설정해서 가상 주소 공간을 물리 주소 공간에 매핑한다.

하지만 이 방법은 메모리를 낭비한다. 메모리를 확보한 때부터 혹은 프로세스가 종료될 때까지 사용하지 않는 영역이 있기 때문이다. **디맨드 페이징**을 사용하면 프로세스의 가상 주소 공간 내의 각 페이지에 대응하는 주소는 처음 접근할 때 할당되지만, 물리 주소는 실제로 가상 주소 영역에 접근할 때 획득한다.

mmap() 시스템 콜 메모리 확보 > 확보한 가상 메모리에 접근하여 물리 메모리 확보 시에 페이지 폴트 > 페이지 폴트 핸들러 >  물리 메모리 할당 > 이 시점에 물리 메모리가 충분하지 않으면 물리 메모리 부족 발생

* 메모리 영역을 획득(mmap())하고 나서 접근할 때까지는 가상 메모리의 사용량이 증가하지만 물리 메모리 사용량은 증가하지 않음
* 메모리 접근을 하면 물리 메모리 사용량은 증가하지만 가상 메모리 사용량은 증가하지 않음
* 메모리 접근이 종료된 뒤 물리 메모리 사용량은 메모리를 획득하기 전과 비교해 증가함
...
가상 메모리 부족은 프로세스가 가상 주소 공간의 범위가 꽉 차도록 가상 메모리를 전부 사용한 뒤에도 가상 메모리를 더 요청했을 때 발생한다. x86_64 아키텍처에서는 가상 주소 공간이 128테라바이트가 있기 때문에 거의 발생하지 않는다.
...
스와핑과 같이 저장 장치에 대한 접근이 발생하는 페이지 폴트를 "Major Fault"라고 하고, 이외를 'Minor Fault' 라고 한다. Major Fault 가 악영향이 더 크다.

</details>

<details>
<summary>스왑(swap)</summary>

스왑은 저장 장치의 일부를 일시적으로 메모리 대신 사용하는 방식이다. 구체적으로는 시스템의 물리 메모리가 부족한 상태가 되어 물리 메모리를 획득할 때에, 기존에 사용하던 물리 메모리의 일부분을 저장 장치에 저장하여 빈 공간을 만들어낸다. 이때 메모리의 내용이 저장된 영역을 스왑 영역이라고 부른다.

문제는 저장 장치에 접근하는 속도가 메모리에 접근하는 속도에 비해 너무나도 느리다는 점이다. 이러한 스와핑이(메모리 부족 현상이) 일시적이 아니라 만성적으로 발생한다면 스래싱 상태가 된다.
